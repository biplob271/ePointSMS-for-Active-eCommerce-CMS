...
class SendSMSUtility
{
    ...
    {        
        if (OtpConfiguration::where('type', 'nexmo')->first()->value == 1) {
            ...
            // curl_close($ch);
                ...
                // added by shahinur
                        ...
                        $tokenSMS= "9|iNSZ1HCodYFyTk9jKsr1Ktg6sCInSqXNG4x7q9sx";  // this is API Token Provided by EPOINT TECHNOLOGIES


                        $postData = [ "sms" => "Kushtia Mart: $text",
                            "number" => $to,
                        ];
		...
		elseif (OtpConfiguration::where('type', 'epoint_sms')->first()->value == 1) {
             $token = env("EPOINT_SMS_API_TOKEN"); //put ssl provided api_token here
			 $sid = env("EPOINT_SMS_SID"); // put ssl provided sid here
